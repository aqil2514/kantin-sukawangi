...
// TODO : Komponen masih belum siap. Ini agak panjang dan lumayan ribet nanganinnya, fokus ke yang lain dulu ajah. Jadi ini komponen yang kalo ada yang mau lanjutin pemesanan, diarahin ke /checkout sambil bawa session.
export default function ContinueSection() {
  const { data, setItems } = useCartContext();
  ...
  const onSubmit = useCallback(
    async (data: FormData) => {
      ...
      try {
        ...
        >("/api/cart", { params: { token: data.token } });
        ...
        if (status === "awaiting_payment" || status === "pending") {
          setResponseMessage(
            "Link pembayaran ditemukan, sedang mengalihkan..."
          );
          reset();
          setCountdown(3);
    ...
    [isFeatureAvailable, reset, router, setItems]
  );
